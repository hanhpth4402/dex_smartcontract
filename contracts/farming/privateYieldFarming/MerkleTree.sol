...
contract MerkleTree {
    ...
    uint32 levels = 32; // level 0 -> 31 there are 32 layers
    uint256[32] public filledNode; /// list of the last node have both child (or last left leaf) in each level
    mapping (uint256 => bool) public isRoot; /// checking availability of this root
    uint256 public lastRoot;

    IHasher public immutable hashFunction = IHasher(0x05392d822a4EA48B95D75DDD53ae846D8808ab73);
    uint256 public nextIndex = 0;

    constructor() {
        for (uint32 i=0; i<levels; i++) {
            filledNode[i] = zeros(i);
        }
    }

    function insert (uint256 leaf) internal returns (uint256) {
        uint256 _nextIndex = nextIndex;
        require((_nextIndex < uint256(2)**32), "merkle tree is full, can not add any leaves more");

        uint256 _currentIndex = _nextIndex;
        uint256 _currentLevelHash = leaf;
        ...
        for (uint32 i=0; i<levels; i++) {
            if (_currentIndex % 2 == 0) {
                ...
                leftChild = _currentLevelHash;
                rightChild = zeros(i);
                filledNode[i] = leftChild; /// left child have both child
            } else {
                //current hash is right leftChild
                rightChild = _currentLevelHash;
                leftChild = filledNode[i];
            }

            _currentLevelHash = hashLeftRight(leftChild, rightChild);
            _currentIndex = _currentIndex / 2;
        }

        isRoot[_currentLevelHash] = true; /// now the new current level hash is the (level-1)th node and it is the new root;

        lastRoot = _currentLevelHash;
        nextIndex += 1;
        return (nextIndex);
    }

    function hashLeftRight (uint256 leftChild, uint256 rightChild) internal view returns (uint256) {
        require(leftChild < FIELD_SIZE, "_left should be inside the field");
        require(rightChild < FIELD_SIZE, "_right should be inside the field");
        uint256 R = leftChild;
        uint256 C = 0;

        (R, C) = hashFunction.MiMCSponge(R, C);
        R = addmod(R, rightChild, FIELD_SIZE);
        (R, C) = hashFunction.MiMCSponge(R, C);
        return R;
    }
